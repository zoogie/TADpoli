...
{
	volatile uint64_t *a64 = a;  //this function was getting nop'd without the volatile declarations (when -O3 was set)
	volatile uint64_t *b64 = b;
	volatile uint64_t tmp = (a64[0]>>1)+(b64[0]>>1) + (a64[0] & b64[0] & 1);
...
void F_XY(volatile uint32_t *key, volatile uint32_t *key_x, volatile uint32_t *key_y) //this function was getting nop'd without the volatile declarations (when -O3 was set)
{
	volatile int i;
	unsigned char key_xy[16];

	memset(key_xy, 0, 16);
	memset((uint32_t*)key, 0, 16);
